...
@implementation ONCardsSwipingView
...
- (void)enqueueCard:(UIView *)card {
    if (self.cards.count > 0) {
        [ONCardsSwipingView prepareToBecomeBackgroundCard:card];
        [self insertSubview:card belowSubview:self.cards.lastObject];
    } else {
        [ONCardsSwipingView prepareToBecomeTopCard:card];
...
- (UIView*)dismissTopCard:(MXCardDestination)destination{
    ...
    switch (destination) {
        case MXCardDestinationLeft:
            ...
            accessoryView = [ONCardsSwipingView viewShownOnSwipeLeftForCard:topCard];
            break;
        case MXCardDestinationRight:
            newAnchor = CGPointMake(self.bounds.size.width + topCard.frame.size.height, topCard.center.y);
            accessoryView = [ONCardsSwipingView viewShownOnSwipeRightForCard:topCard];
            break;
        case MXCardDestinationUp:
            newAnchor = CGPointMake(topCard.center.x, -topCard.frame.size.height);
            accessoryView = [ONCardsSwipingView viewShownOnSwipeUpForCard:topCard];
            break;
        default:
            NSLog(@"default");
            newAnchor = CGPointMake(self.bounds.size.width + topCard.frame.size.height, topCard.center.y);
...
- (void)showLeftOrRightOrUpViewsOnCard:(UIView*)card {
    UIView* leftView = [ONCardsSwipingView viewShownOnSwipeLeftForCard:card];
    UIView* rightView = [ONCardsSwipingView viewShownOnSwipeRightForCard:card];
    UIView* upView = [ONCardsSwipingView viewShownOnSwipeUpForCard:card];
...
- (void)dismissCard:(UIView*)card toPoint:(CGPoint)newAnchor viaAttachment:(UIAttachmentBehavior*)attachment {
    ...
    if (nextCard) {
        [UIView animateWithDuration:0.3 animations:^{
            [ONCardsSwipingView prepareToBecomeTopCard:nextCard];
        }];
    }
}
...
- (void)restoreTopCardToCenter {
    ...
    UIView* topCard = [self topCard];
    UIView* leftView = [ONCardsSwipingView viewShownOnSwipeLeftForCard:topCard];
    UIView* rightView = [ONCardsSwipingView viewShownOnSwipeRightForCard:topCard];
    UIView* upView = [ONCardsSwipingView viewShownOnSwipeUpForCard:topCard];
